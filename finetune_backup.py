...
def main(config):
  ...
  if config['LP']:
    ...
    elif config.get('LP_FT') and config.get('path'):
      ### load LP
      assert os.path.exists(os.path.join(config['path'], config['ckpt']))
      ckpt = torch.load(os.path.join(config['path'], config['ckpt']))
  ...
  if not config['LP']:
    ### NC optimizer
    utils.log("load optimizer for multitask FT")
    optimizer = optimizers.make(config['optimizer'], model.parameters(), 
                            **config['optimizer_args'])
  elif not config.get('LP_FT'):
    ### LP from scratch, fix encoder
  ...
  for epoch in range(start_epoch, config['n_epochs'] + 1):
    ...
    if not config['LP']:
      ...
      model_lp.enc.eval()
      # adjust learning rate
      lr_lp = utils.decay_lr(epoch, config['n_epochs'], **config['lp_optimizer_args'])
      ...
      eval_val = True # do not run eval on fs-centroid
      model_lp.train()
      ## change BatchNorm:
      if "RN" in ckpt['encoder']:
          enc.apply(utils.set_bn_eval)
    ...
    if config['LP']:
      ckpt['lp'] = 'logistic'
      ckpt['lp_args'] = config.get('lp_args') or dict()
      ckpt['lp_state_dict'] = model_lp.head.state_dict()
      if not config['LP_FT']:
        ckpt['lp_optimizer'] = config['lp_optimizer']
        ckpt['lp_optimizer_args'] = config['optimizer_args']
        ckpt['lp_optimizer_state_dict'] = lp_optimizer.state_dict()
